...
class ImageTagger:
    ...
    async def _process_image_async(self, image_path, labels, threshold):
        """
        Asynchronous forward pass through the CLIP model for a single image
        with given text labels, returning labels above a probability threshold.
        ...
        try:
            # Load + preprocess in a thread
            image = await loop.run_in_executor(None, self._prepare_image, image_path)
            text = clip.tokenize(labels).to(self.device)
            ...
            logger.error(f"Error processing image {image_path} (async): {str(e)}")
            return []
    ...
    async def _load_images_async(self, images_path):
        """
        Asynchronously loads and preprocesses all images from a directory,
        using a ThreadPoolExecutor for I/O and preprocessing in parallel.
        ...
        with ThreadPoolExecutor() as pool:
            for img_path in image_files:
                try:
                    img_tensor = await loop.run_in_executor(pool, self._prepare_image, img_path)
                    images.append(img_tensor)
                except Exception as e:
                    logger.error(f"Cannot open image {img_path} async: {e}")
    ...
    async def tag_image_async(self, image_path, output_path=None, top_k=5, labels=None, threshold=0.3):
        """
        Asynchronously generates tags for a given image using CLIP, optionally saves metadata.

        Args:
            image_path (str): Path to the image.
            output_path (str, optional): Path to save metadata.
            top_k (int, optional): Number of top tags to return. Defaults to 5.
            labels (list, optional): List of possible tags. Defaults to default_labels.
            threshold (float, optional): Probability threshold. Defaults to 0.3.
        ...
        results = await self._process_image_async(image_path, labels, threshold)
        results = sorted(results, key=lambda x: x["probability"], reverse=True)[:top_k]
    ...
    async def generate_tags_async(self, image_path, labels=None, threshold=0.3):
        """
        Asynchronously generates a list of tags for a given image, returning only the tag names.
    ...
    async def group_duplicates_async(self, duplicates, output_folder, operation="copy"):
        """
        Groups duplicate images into subfolders named after their most frequent tag (asynchronous).

        Args:
            duplicates (list[tuple]): List of tuples (img1, img2, similarity).
            output_folder (str): Where grouped images will be placed.
            operation (str, optional): File operation ('copy', 'symlink', etc.). Defaults to "copy".
        """
        grouped = {}
        for img1, img2, similarity in duplicates:
            group_id = None
        ...
        for group_id, images_ in grouped.items():
            tag_samples = []
            # Asynchronously gather tags
            for image_path in images_:
                tags = await self.generate_tags_async(image_path)
                tag_samples.extend(tags)
            ...
            # The file operation is sync, but we could async-ify it if needed
            for image_path in images_:
                perform_file_operation(image_path, group_folder, operation)
        
        logger.info(f"Grouped duplicates into {len(grouped)} folders with meaningful tags (async).")
        click.echo(f"Grouped duplicates into {len(grouped)} folders with meaningful tags (async).")
    
    # ----------------------------------------------------------
    # Search images (sync + async)
    # ----------------------------------------------------------
    ...
    async def search_images_async(self, query, images_path, top_k=5):
        """
        Asynchronously searches for images most similar to a text query using CLIP embeddings.
        ...
        Returns:
            list[tuple(str, float)]: A list of (image_path, similarity).
        """
        image_files, image_tensors = await self._load_images_async(images_path)
        if len(image_files) == 0 or image_tensors.shape[0] == 0:
            logger.warning("No images to search in (async).")
            return []
    ...
    # ----------------------------------------------------------
    # Find duplicates (sync + async) with progress
    # ----------------------------------------------------------
    ...
    async def find_duplicates_async(self, images_path, similarity_threshold=0.9):
        """
        Asynchronously identifies duplicate images based on cosine similarity of their embeddings,
        printing progress in one line with 'Comparing X/Y pairs'.
        ...
        image_files, image_tensors = await self._load_images_async(images_path)
        if len(image_files) == 0 or image_tensors.shape[0] == 0:
            logger.warning("No images found for duplicate checking (async).")
            return []
        ...
        num_images = len(image_files)
        total_pairs = (num_images * (num_images - 1)) // 2
        click.echo("Checking duplicates (async) ...")
        
        current_pair = 0
        for i in range(num_images):
            for j in range(i + 1, num_images):
                current_pair += 1
                msg = f"Comparing pairs: {current_pair}/{total_pairs}"
                click.echo(msg, nl=False)  # no new line
